...
- **표현**과 **연산**
  - **표현**:
    - 보수 방식에 따라 숫자를 나타내는 것
    - 4비트에서 `1000`은 `-8`이 메모리에 저장되는 방식이다
    - 4비트에서 `1000`을 `-8`로 나타내기 위해 `NOT 7 = -7 - 1 = -8`이 되는 것
  - **연산**:
    - 보수 방식에 따라 실제 연산을 하는 것
    - 보수를 구하기 위해 `비트를 뒤집고 +1하는 것`
...
- $1 \to 0$ 또는 $0 \to 1$로 비트를 뒤집은 후 `+1`을 해준다
- 오버플로우?
...
- 비트 내에서 0과 1을 바꿔준다
- 4비트 체계에서 `~0111`은 `1000`이 되며, 2의 보수에서 `1000`은 `NOT x = -x - 1` = `NOT 7 = -7 - 1` = `-8`로 표현된다
- `0b0101 ^ ~0b1100`, 여기서 8비트로 보고 다시 확인
